...
function RHPlusLogo() {
  return (
    <svg width="48" height="48" viewBox="0 0 48 48" xmlns="http://www.w3.org/2000/svg" style={{borderRadius:12,boxShadow:"0 2px 12px rgba(14,165,233,0.4)"}}>
      <circle cx="24" cy="24" r="24" fill="#0C2D4E"/>
      <circle cx="24" cy="24" r="22" fill="none" stroke="#0EA5E9" strokeWidth="1" opacity="0.4"/>
      <text x="26" y="28" textAnchor="middle" fontFamily="Georgia, serif" fontSize="14" fontWeight="700" fill="#7DD3FC" opacity="0.25" letterSpacing="-0.5">RH+</text>
      <text x="25" y="27" textAnchor="middle" fontFamily="Georgia, serif" fontSize="14" fontWeight="700" fill="#BAE6FD" opacity="0.3" letterSpacing="-0.5">RH+</text>
      <text x="24" y="26" textAnchor="middle" fontFamily="Georgia, serif" fontSize="14" fontWeight="700" fill="white" letterSpacing="-0.5">RH+</text>
    </svg>
  );
}
...
function SkeletonCard() {
  return (
    <div style={{background:"rgba(255,255,255,0.96)",borderRadius:16,padding:"28px 32px",boxShadow:"0 2px 20px rgba(15,23,42,0.08)",border:"1.5px solid rgba(186,230,253,0.55)"}}>
      ...
      <div style={{height:16,background:"#E2E8F0",borderRadius:8,width:"75%",marginBottom:20}}/>
...
function IPCWidget() {
  ...
  if(loading) return <div style={{background:"#1E3A5F",borderRadius:16,padding:"24px 28px",marginBottom:28,height:140}}/>;
  if(!ipc) return null;
...
function ShareButton({news}) {
  const [shared,setShared]=useState(false);
  const handleShare=async()=>{
    const text=`${news.title}\n\n${news.summary?.slice(0,200)}...\n\nFuente: ${news.source}\n${news.url}`;
    if(navigator.share){
      try{ await navigator.share({title:news.title,text,url:news.url}); setShared(true); setTimeout(()=>setShared(false),2000); }catch{}
    } else {
  ...
  return (
    <button onClick={handleShare} style={{padding:"12px 20px",borderRadius:10,border:"2px solid",borderColor:shared?"#0D9488":"#CBD5E1",background:shared?"#F0FDFA":"transparent",color:shared?"#0D9488":"#64748B",fontSize:15,fontWeight:700,cursor:"pointer",display:"flex",alignItems:"center",gap:6}}>
      ...
      <div style={{paddingLeft:14}}>
        ...
        {news.image&&!compact&&(
          <div style={{marginBottom:14,borderRadius:10,overflow:"hidden",maxHeight:160}}>
            <img src={news.image} alt={news.title} style={{width:"100%",height:160,objectFit:"cover"}} onError={e=>e.target.style.display="none"}/>
          </div>
        )}

        <h3 style={{margin:"0 0 14px",fontSize:compact?16:20,fontWeight:700,color:"#0F172A",lineHeight:1.45}}>{news.title}</h3>
        <div style={{margin:"0 0 18px",fontSize:compact?14:16,color:"#1E3A5F",lineHeight:1.82}}>
          {shown}
          {isLong&&!compact&&(
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,flexWrap:"wrap",borderTop:"1px solid #F1F5F9",paddingTop:16}}>
          <a href={news.url} target="_blank" rel="noopener noreferrer"
            style={{display:"inline-flex",alignItems:"center",gap:9,padding:"12px 22px",background:bg||"#DBEAFE",border:`1.5px solid ${accent||"#0369A1"}44`,borderRadius:10,color:accent||"#0369A1",fontSize:15,fontWeight:700,textDecoration:"none"}}>
            🔗 Ver noticia · {news.source}
          </a>
          <div style={{display:"flex",gap:8}}>
            {!compact&&<ShareButton news={news}/>}
            <button onClick={onToggleLike} style={{padding:"12px 22px",borderRadius:10,border:"2px solid",borderColor:isLiked?"#0EA5E9":"#CBD5E1",background:isLiked?"#EFF6FF":"transparent",color:isLiked?"#0369A1":"#64748B",fontSize:15,fontWeight:700,cursor:"pointer",display:"flex",alignItems:"center",gap:6}}>
...
export default function RHPlusApp() {
  ...
  const [likes,setLikes]=useState(()=>{try{return JSON.parse(localStorage.getItem("rh_likes_v1")||"{}");}catch{return {};}});
  ...
  useEffect(()=>{try{localStorage.setItem("rh_likes_v1",JSON.stringify(likes));}catch{}},[likes]);
  ...
  return (
    <div style={{minHeight:"100vh",background:"linear-gradient(160deg,#EFF6FF 0%,#F0FDFA 60%,#E0F2FE 100%)",fontFamily:"'Palatino Linotype','Book Antiqua',Palatino,Georgia,serif"}}>
        ...
        {view==="ipc"&&(
          <>
            ...
            <div style={{background:"rgba(255,255,255,0.8)",borderRadius:14,padding:"20px 24px",border:"1.5px solid #BFDBFE"}}>
              <h3 style={{margin:"0 0 12px",fontSize:17,color:"#1E40AF",fontWeight:700}}>Por que es clave para RH+</h3>
              <p style={{margin:"0 0 10px",fontSize:15,color:"#334155",lineHeight:1.75}}>El IPC es el indicador de referencia para negociaciones paritarias, actualizacion de escalas salariales, ajuste de beneficios y planificacion de presupuestos de compensaciones en Argentina.</p>
              <a href="https://www.indec.gob.ar/indec/web/Nivel4-Tema-3-5-31" target="_blank" rel="noopener noreferrer"
                style={{display:"inline-flex",alignItems:"center",gap:8,padding:"12px 24px",background:"linear-gradient(135deg,#1E3A5F,#1E40AF)",borderRadius:10,color:"white",fontSize:15,fontWeight:700,textDecoration:"none",marginTop:8}}>
            ...
            <div style={{background:`linear-gradient(135deg,${activeCat?.accent||"#0369A1"} 0%,${activeCat?.accent||"#0369A1"}CC 100%)`,borderRadius:16,padding:"24px 32px",marginBottom:28,display:"flex",alignItems:"center",gap:16,boxShadow:`0 4px 20px ${activeCat?.accent||"#0369A1"}44`}}>
              ...
              <div>
                ...
                <p style={{margin:"4px 0 0",color:"rgba(255,255,255,0.78)",fontSize:15}}>
                  Noticias reales en tiempo real · {new Date().toLocaleDateString("es-AR",{month:"long",year:"numeric"})}
                </p>
              </div>
            </div>
      ...
      <footer style={{marginTop:56,borderTop:"1px solid #BFDBFE",padding:"28px 16px",textAlign:"center",background:"rgba(255,255,255,0.6)"}}>
        <p style={{margin:"0 0 2px",fontSize:20,fontWeight:700,color:"#0369A1"}}>RH+</p>
        <p style={{margin:"0 0 10px",fontSize:12,color:"#0D9488",letterSpacing:"1px",fontStyle:"italic",fontWeight:600}}>by JMM</p>
        <p style={{margin:0,fontSize:13,color:"#94A3B8"}}>Noticias reales en tiempo real · NewsAPI · INDEC · Fuentes verificadas de Recursos Humanos</p>
      </footer>
