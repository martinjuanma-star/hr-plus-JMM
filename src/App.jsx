...
const MOD_COLORS = { "Presencial":{bg:"#D1FAE5",color:"#065F46"}, "Virtual":{bg:"#EFF6FF",color:"#1D4ED8"}, "Presencial + Virtual":{bg:"#FEF3C7",color:"#92400E"}, "Presencial + Streaming":{bg:"#FEF3C7",color:"#92400E"} };
...
function SkeletonCard() {
  return (
    <div style={{background:"rgba(255,255,255,0.96)",borderRadius:16,padding:"28px 32px",boxShadow:"0 2px 20px rgba(15,23,42,0.08)",border:"1.5px solid rgba(186,230,253,0.55)",animation:"pulse 1.5s infinite"}}>
  ...
  const fetchNews = useCallback(async (category, force=false) => {
    ...
    try {
      ...
      if (data.success && data.news?.length > 0) {
        setNewsCache(prev => ({...prev, [category]: data.news}));
      } else {
        throw new Error("Sin resultados");
      }
    } catch(e) {
      setError("No se pudieron cargar las noticias en este momento. Intenta de nuevo.");
    }
    setLoading(false);
  }, [newsCache]);
  ...
  return (
    <div style={{minHeight:"100vh",background:"linear-gradient(160deg,#EFF6FF 0%,#F0FDFA 60%,#E0F2FE 100%)",fontFamily:"'Palatino Linotype','Book Antiqua',Palatino,Georgia,serif"}}>
          ...
          <>
            ...
            <div style={{background:`linear-gradient(135deg,${activeCat?.accent||"#0369A1"} 0%,${activeCat?.accent||"#0369A1"}CC 100%)`,borderRadius:16,padding:"24px 32px",marginBottom:28,display:"flex",alignItems:"center",gap:16,boxShadow:`0 4px 20px ${activeCat?.accent||"#0369A1"}44`}}>
              ...
              <div>
                ...
                <p style={{margin:"4px 0 0",color:"rgba(255,255,255,0.78)",fontSize:15}}>
                  Noticias en tiempo real · fuentes globales verificadas · {new Date().toLocaleDateString("es-AR",{month:"long",year:"numeric"})}
                </p>
              </div>
            </div>
            ...
            {error&&(
              <div style={{background:"#FEF2F2",border:"1.5px solid #FECACA",borderRadius:12,padding:"20px 24px",marginBottom:24,color:"#DC2626",fontSize:15}}>
                {error}
                <button onClick={handleRefresh} style={{marginLeft:16,background:"#DC2626",color:"white",border:"none",borderRadius:8,padding:"8px 16px",fontSize:14,fontWeight:700,cursor:"pointer"}}>
                  Reintentar
                </button>
              </div>
            )}
      ...
      <footer style={{marginTop:56,borderTop:"1px solid #BFDBFE",padding:"28px 16px",textAlign:"center",background:"rgba(255,255,255,0.6)"}}>
        ...
        <p style={{margin:0,fontSize:13,color:"#94A3B8"}}>Noticias en tiempo real de SHRM · HBR · MIT Sloan · Gallup · Josh Bersin · AIHR · Employer Brand International</p>
      </footer>
